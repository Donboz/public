...
exports.run = async (client, message, args) => {
  ...
  const ravi2 = new MessageEmbed().setFooter("").setColor(Settings.Colors.Red).setAuthor(message.author.username, message.author.avatarURL({ dynamic: true }))
  ...
  if(!users) return message.channel.send(embedx.setDescription(`Bir Üye Etiketlemelisin.`).setFooter("Bir Şey Mi Unuttun?").setTimestamp().setColor("RED"))
...
users.roles.remove(r.id)
  ...
  message.channel.send(embed.setDescription(`${users} Adlı Kullanıcı Başarıyla Kayıtsız'a Atıldı.`).setTimestamp().setColor("GREEN").setFooter("Aferin"))
}
module.exports.conf = {
  enabled: true,
  guildOnly: true,
