...
user.roles.add(Settings.Roles.KızRol)
...
let toplam = await db.get(`${message.author.id}.toplam`)
  ...
  let embed = new MessageEmbed()
    ...
    • <@&${Settings.Roles.UyeRolu}>, <@&${Settings.Roles.KızRol}> Başarıyla Verildi.
    • Kayıt Sırası: **#${sıra || "1"}**
`)
.setFooter(`${message.author.username} Yetkilisinin Toplam ${toplam} Kaydı Oldu.`)
.setTimestamp()
client.channels.cache.get(Settings.Channels.KayitLog).send(embed)
